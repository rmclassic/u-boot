...
typedef struct nvboot_badblock_table_rec {
	u32 entries_used;
	u8 virtual_blk_size_log2;
	u8 block_size_log2;
	u8 bad_blks[NVBOOT_BAD_BLOCK_TABLE_SIZE / 8];
...
typedef struct nvboot_config_table_rec {
	...
	u8 customer_data[NVBOOT_BCT_CUSTOMER_DATA_SIZE];
	/*
	...
	u32 reserved1;
	u8 enable_fail_back;
	u8 reserved[NVBOOT_BCT_RESERVED_SIZE];
} nvboot_config_table;
